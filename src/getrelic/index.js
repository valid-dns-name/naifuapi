...
exports.handler = async (event, context) => {
  ...
  item = ddbHashMapParamsGetDAO(
    "relic_id",
    dynamodb,
    process.env.RELICS,
    event.queryStringParameters.token_id
  );
  if (item) {
    let relic = {
      name: item.name,
    ...
    let update_used_relic = ddbHashMapParamsPushDAO(
      "relic_id",
      dynamodb,
      process.env.RELICS,
      relic_id
    );
    let ddb_update = ddbHashMapParamsPushDAO(
      "token_id",
      dynamodb,
      process.env.RELIC_TOKENS,
      relic
    );
...
function getRelicRarityCollection(rarity, dynamodb) {
  let rarity = ddbHashMapParamsGetDAO(
    "rarity",
    dynamodb,
    process.env.RELIC_COLLECTIONS,
    rarity
  );
...
function getRelicForRarity(collection, dynamodb) {
  ...
  let relic = ddbHashMapParamsGetDAO(
    "relic_id",
    dynamodb,
    process.env.RELICS,
    relic_id
