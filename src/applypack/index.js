...
import { ddbHashMapParamsGetDAO, ddbHashMapParamsPushDAO, rollRarity } from "../utils/dao"
exports.handler = async (event, context) => {
  const dynamodb = new AWS.DynamoDB.DocumentClient()
  // Log the event argument for debugging and for use in local development.
  console.log(JSON.stringify(event, undefined, 2));
  naifu_id = event.naifu_id
  let naifu_response = ddbHashMapParamsGetDAO("token_id", dynamodb, process.env.USER_NAIFUS, naifu_id)
  if (naifu_response) {
    let old_naifu = lodash.cloneDeep(naifu_response.Item)
    try {
    let naifu = naifu_response.Item
    let cardrarities = {}
    for (let i = 0; i < 51; i++) {
    ...
    for (const [rarity, quantity] of Object.entries(cardrarities)) {
        rarity_collection = JSON.parse(ddbHashMapParamsGetDAO("rarity", dynamodb, process.env.CARD_RARITIES, rarity));
    ...
    let ddb_naifu_update = ddbHashMapParamsPushDAO("token_id", dynamodb, process.env.USER_NAIFUS, naifu)
    let successresponse = {
      statusCode: 200,
      headers: {
  ...
  try {
  let rollback_update = ddbHashMapParamsPushDAO('token_id', dynamodb, process.env.USER_NAIFUS, old_naifu)
  } catch(error) {
    console.log(error)
    logging_naifu = JSON.stringify(old_naifu)
    console.log(`We really broke this naifu, naifu old_state, please roll it back ASAP ${logging_naifu}`)
    let rollbackfailureresponse = {
      statusCode: 500,
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ "message": "Rollback failed!", "rollback": false})
    }
  ...
  let failureresponse = {
    ...
    body: JSON.stringify( {"message": "Failed to write new naifu", "old_naifu": JSON.stringify(old_naifu), "new_naifu": JSON.stringify(naifu), "rollback": true })
  }
